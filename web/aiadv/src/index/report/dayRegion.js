 ...
 */
(function () {
    mf.index.report.dayRegion = new er.Action({
        model: mf.index.report.model.dayRegion,
        view: new er.View({
            template: 'mf_index_report_dayRegion',
            UI_PROP: {
                dayRegionDate: {
        ...
        onafterrender: function () {
            ...
            var date = esui.get('dayRegionDate');
            ...
            var dayRegionListTable = esui.get('dayRegionList');
            dayRegionListTable.onsort = mf.m.utils.nextTickWrapper(function (orderField, order) {
                var orderBy = orderField.field;
                order = order == 'asc' ? 1 : -1;
                dayRegionListTable.datasource.sort(function (a, b) {
                    return a[orderBy] > b[orderBy] ? order : -order;
                });
                dayRegionListTable.render();
            }, dayRegionListTable);
        },
        onentercomplete: function () {
            ...
            var action = this;
            var model = action.model;
            var $element = $('#dayRegionChart');
            mf.m.highchart_drill('#dayRegionChart',
                model.get('lists'), model.get('date'),
                function (event) {
                    if (event.point.name) {
                        $element.attr({
                            'data-cmd': 'step_province',
                            'data-province': event.point.province_id,
            ...
            mf.mockPager(data, {
                pager: esui.get('dayRegionPager'),
                pageSizer: esui.get('dayRegionPageSize'),
                table: esui.get('dayRegionList')
            }, {
