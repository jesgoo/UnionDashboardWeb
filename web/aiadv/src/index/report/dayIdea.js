 ...
 */
(function () {
    mf.index.report.dayIdea = new er.Action({
        model: mf.index.report.model.dayIdea,
        view: new er.View({
            template: 'mf_index_report_dayIdea',
            UI_PROP: {
                ayUnitDate: {
        ...
        onafterrender: function () {
            ...
            var dayIdeaListTable = esui.get('dayIdeaList');
            dayIdeaListTable.onsort = mf.m.utils.nextTickWrapper(function (orderField, order) {
                var orderBy = orderField.field;
                order = order == 'asc' ? 1 : -1;
                dayIdeaListTable.datasource.sort(function (a, b) {
                    return a[orderBy] > b[orderBy] ? order : -order;
                });
                dayIdeaListTable.render();
            }, dayIdeaListTable);
        },
        onentercomplete: function () {
            ...
            var action = this;
            var model = action.model;
            var $element = $('#dayIdeaChart');
            mf.m.highchart_drill('#dayIdeaChart',
            //mf.m.highchart_drill_cost('#dayIdeaChart',
                model.get('lists'), model.get('date'),
                function (event) {
                    if (event.point.name) {
                        $element.attr({
                            'data-cmd': 'step_idea',
                            'data-idea': event.point.idea_id,
            ...
            mf.mockPager(data, {
                pager: esui.get('dayIdeaPager'),
                pageSizer: esui.get('dayIdeaPageSize'),
                table: esui.get('dayIdeaList')
            }, {
