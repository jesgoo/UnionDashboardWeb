 ...
 */
(function () {
    mf.admin.manage.userDSPID = new er.Action({
        model: mf.admin.manage.model.userDSPID,
        view: new er.View({
            template: 'mf_admin_manage_userDSPID',
            UI_PROP: {}
        }),
        STATE_MAP: {
            page: 0,
        ...
        onafterrender: function () {
            ...
            var userDSPList = model.get('userDSPList');
            var DSPFieldInConfig = mf.mockFieldInConfig(userDSPList);
            var operateData = mf.operateDataInConfigField(userDSPList);
            var emptyDSP = mf.initEntityInConfig(userDSPList);

            var table = esui.get('list');
            table.order = 'asc';
            table.orderBy = DSPFieldInConfig('user');
            ...
            var saveRow = function (rowIndex) {
                ...
                if (mf.tableSavingValidator(row, table.fields)) {
                    mf.parallelAjax({
                        type: 'POST',
                        url: '/admin/useradslot' + (
                            row._isNew ? '' : '/' + operateData.get(row, 'user')
                             + '/' + operateData.get(row, 'type')
                             + '/' + operateData.get(row, 'dsp_type')
                             + '/' + operateData.get(row, 'platform')),
                        data: mf.grepDataInConfig(row, userDSPList)
                    }, function (result) {
                        var newData = result[0];
                        if (row._isNew) {
                            dataList.unshift(newData);
                        } else {
                            var idField = DSPFieldInConfig('user');
                ...
                mf.clickCommand.register(
                    [
                        ...
                        },
                        {
                            cmd: 'search',
                            handle: function (options) {
                                var text = esui.get('userID').getValue();
                                var filter;
                                if (text) {
                                    var valueRegExp = mf.m.utils.makeRegExp(text, 'i');
                                    filter = function (obj) {
                                        return valueRegExp.test(operateData.get(obj, 'user'))
                        ...
                        },
                        {
                            ...
                            handle: function (options) {
                                ...
                                newRow = mf.grepDataInConfig(newRow, userDSPList);
                                newRow._isNew = true;
