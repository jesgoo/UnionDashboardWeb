 ...
 */
(function () {
    mf.admin.manage.mediaDSPID = new er.Action({
        model: mf.admin.manage.model.mediaDSPID,
        view: new er.View({
            template: 'mf_admin_manage_mediaDSPID',
            UI_PROP: {}
        }),
        STATE_MAP: {
            page: 0,
        ...
        onafterrender: function () {
            ...
            var mediaDSPList = model.get('mediaDSPList');
            var DSPFieldInConfig = mf.mockFieldInConfig(mediaDSPList);
            var operateData = mf.operateDataInConfigField(mediaDSPList);
            var emptyDSP = mf.initEntityInConfig(mediaDSPList);

            var table = esui.get('list');
            table.order = 'asc';
            table.orderBy = DSPFieldInConfig('media');
            ...
            var saveRow = function (rowIndex) {
                ...
                if (mf.tableSavingValidator(row, table.fields)) {
                    mf.parallelAjax({
                        type: 'POST',
                        url: '/admin/mediaadslot' + (
                            row._isNew ? '' : '/' + operateData.get(row, 'media')
                             + '/' + operateData.get(row, 'type')
                             + '/' + operateData.get(row, 'dsp_type')
                             + '/' + operateData.get(row, 'platform')),
                        data: mf.grepDataInConfig(row, mediaDSPList)
                    }, function (result) {
                        var newData = result[0];
                        if (row._isNew) {
                            dataList.unshift(newData);
                        } else {
                            var idField = DSPFieldInConfig('media');
                ...
                mf.clickCommand.register(
                    [
                        ...
                        },
                        {
                            cmd: 'search',
                            handle: function (options) {
                                var text = esui.get('mediaID').getValue();
                                var filter;
                                if (text) {
                                    var valueRegExp = mf.m.utils.makeRegExp(text, 'i');
                                    filter = function (obj) {
                                        return valueRegExp.test(operateData.get(obj, 'media'));
                                    };
                                }
                                refreshTable({
                                    page: 0,
                        ...
                        },
                        {
                            ...
                            handle: function (options) {
                                ...
                                newRow = mf.grepDataInConfig(newRow, mediaDSPList);
                                newRow._isNew = true;
